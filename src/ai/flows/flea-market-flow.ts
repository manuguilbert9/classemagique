...
export const fleaMarketFlow = ai.defineFlow(
    ...
    async (input: FleaMarketInput) => {
        const { itemName, studentPrice, level } = input;

        const prompt = `
      Tu es un client dans une brocante organisée par une classe d'école primaire.
    ...
    `;
        ...
        if (!object) {
            throw new Error('Failed to generate flea market response');
        }
