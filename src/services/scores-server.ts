...
export interface ScoreDetail {
     ...
     * 'corrected' : faux d'abord, puis la bonne réponse a été trouvée, avec ou sans aide. Ne rapporte aucun point, mais mérite d'être
...
export interface Score {
    id: string;
    sessionId?: string;
    metadata?: {tables?: number[]; level?: string; unit?: 'percent' | 'MCLM' | 'count' | 'completion'; durationSeconds?: number; mode?: string; assistance?: string[]; text?: string; wordsRead?: number; errors?: number};
    context?: 'classroom' | 'homework';
    homeworkDate?: string;
    userId: string;
    skill: string;
    score: number;
...
export async function persistResult(scoreData: Omit<Score, 'id' | 'createdAt'> & { sessionId: string }): Promise<{success: boolean; error?: string; nuggetsEarned?: number}> {
    if (!scoreData.userId || !scoreData.sessionId || !Number.isFinite(scoreData.score)) return {success:false,error:'Résultat invalide.'};
    try {
        const scoreRef = doc(db, 'scores', scoreData.sessionId);
        ...
        await runTransaction(db, async transaction => {
            ...
            if (!student.exists()) throw new Error('Élève introuvable.');
            const rewardScore = scoreData.metadata?.unit === 'count'
                ? (scoreData.details?.length ? 100 * scoreData.details.filter(d => d.status === 'correct').length / scoreData.details.length : 0)
                : scoreData.score;
            nuggetsEarned = calculateNuggets(rewardScore, scoreData.skill);
            const data = JSON.parse(JSON.stringify({...scoreData, details: scoreData.details || []}));
            const createdAt = Timestamp.now();
            transaction.set(scoreRef, {...data, createdAt});
            if (scoreData.homeworkDate) transaction.set(doc(db, 'homeworkResults', scoreData.sessionId), {...data, skillSlug:scoreData.skill, date:scoreData.homeworkDate, createdAt});
            transaction.update(studentRef, {nuggets: (student.data().nuggets || 0) + nuggetsEarned});
        });
        return {success:true,nuggetsEarned};
    } catch (error) { return {success:false,error:error instanceof Error ? error.message : 'Enregistrement impossible.'}; }
}
...
export async function deleteScore(scoreId: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        batch.delete(doc(db, 'homeworkResults', scoreId));
        await batch.commit();
        return { success: true };
    } catch (error) {
