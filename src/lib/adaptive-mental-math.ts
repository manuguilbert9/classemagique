...
export interface MentalMathCompetency {
    ...
    getHelp?: (question: Question) => HelpData;
}
...
const allCompetencies: MentalMathCompetency[] = [
    // --- Level A ---
    {
        ...
        getHelp: (q) => {
            ...
            return { type: 'number-line', start: Math.max(0, n - 2), end: n + 2, highlight: n, jump: 1 };
        }
    },
    {
        id: 'A2',
    ...
    },
    {
        id: 'A3',
        level: 'A',
        description: 'Dénombrer des collections jusqu\'à 10',
        generate: () => { const count = randInt(1, 10); return { question: `Combien y a-t-il de cercles ? ${'● '.repeat(count)}`, answer: String(count) }; },
        getHelp: (q) => {
            const count = parseInt(q.answer as string);
            return { type: 'visual-dots', count1: count, operation: 'count' };
        }
    },
    {
        id: 'A4',
    ...
    },
    {
        id: 'A5',
        level: 'A',
        description: 'Maîtriser les compléments à 10',
        generate: () => { const a = randInt(1, 9); return { question: `${a} + ? = 10`, answer: String(10 - a) } },
        getHelp: (q) => {
            const a = parseInt(q.question.match(/\d+/)?.[0] || '0');
            return { type: 'visual-dots', count1: a, count2: 10 - a, operation: 'add', label1: 'J\'ai', label2: 'Il manque' };
        }
    },
    {
        id: 'A6',
        level: 'A',
        description: 'Maîtriser les compléments à 5',
        generate: () => { const a = randInt(1, 4); return { question: `${a} + ? = 5`, answer: String(5 - a) } },
        getHelp: (q) => {
            const a = parseInt(q.question.match(/\d+/)?.[0] || '0');
            return { type: 'visual-dots', count1: a, count2: 5 - a, operation: 'add', label1: 'J\'ai', label2: 'Il manque' };
        }
    },
    {
        id: 'A7',
        level: 'A',
        description: 'Ajouter ou retirer 5',
        generate: () => { const a = randInt(6, 20); return Math.random() > 0.5 ? { question: `${a} + 5 = ?`, answer: String(a + 5) } : { question: `${a} - 5 = ?`, answer: String(a - 5) }; },
        getHelp: (q) => {
            const parts = q.question.match(/(\d+) (.) (\d+)/);
            if (!parts) return { type: 'text-hint', text: 'Utilise tes doigts ou imagine des jetons.' };
            const [_, a, op, b] = parts;
            return { type: 'visual-dots', count1: parseInt(a), count2: parseInt(b), operation: op === '+' ? 'add' : 'sub' };
        }
    },
    {
        id: 'A8',
        level: 'A',
        description: 'Ajouter ou retirer 10',
        generate: () => { const a = randInt(11, 20); return Math.random() > 0.5 ? { question: `${a} + 10 = ?`, answer: String(a + 10) } : { question: `${a} - 10 = ?`, answer: String(a - 10) }; },
        getHelp: (q) => {
            const parts = q.question.match(/(\d+) (.) (\d+)/);
            if (!parts) return { type: 'text-hint', text: 'Ajouter 10, c\'est ajouter une dizaine.' };
            const [_, a, op] = parts;
            const num = parseInt(a);
            return { type: 'visual-blocks', tens: Math.floor(num / 10), units: num % 10 };
        }
    },
...
export async function generateAdaptiveMentalMathQuestion(performance: StudentPerformance): Promise<Question> {
    ...
    // Generate help data if available
    const helpData = competency.getHelp ? competency.getHelp(questionData as Question) : undefined;
    ...
    return {
        ...
        help: helpData, // Pass help data to the frontend
        ...questionData
    };
}
...
export async function getAdaptiveMentalMathCompetencies(): Promise<DisplayableMentalMathCompetency[]> {
    ...
    return allCompetencies.map(({ generate, getHelp, ...rest }) => rest);
}
