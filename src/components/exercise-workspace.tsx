...
const motivationalMessages = [
  "Excellent travail !", "Bravo !", "Incroyable !", "Continue comme ça !", "Fantastique !", "Bien joué !"
];
const icons = [
...
export function ExerciseWorkspace({ skill, isTableauMode = false }: ExerciseWorkspaceProps) {
  ...
  // State for count
  const [selectedCountIndices, setSelectedCountIndices] = useState<number[]>([]);
    ...
    async function loadNonConfigurableQuestions() {
        if (!['calculation', 'currency', 'time', 'denombrement', 'lire-les-nombres', 'mental-calculation', 'keyboard-count', 'chemin-des-accords'].includes(skill.slug)) {
            ...
            setIsReadyToStart(true);
             ...
             // These depend on student level but don't have a settings screen
  ...
  }, [skill.slug, isUserLoading, student]);
  ...
  // Automatically speak the question if textToSpeak is available
  useEffect(() => {
    if (exerciseData?.textToSpeak) {
      setTimeout(() => handleSpeak(exerciseData.textToSpeak!), 300);
    }
  }, [exerciseData, handleSpeak]);
  ...
  const handleKeyboardCountKeystroke = useCallback((key: string) => {
      ...
      let newAnswer = userKeyboardInput;
       if (key === 'Backspace' || key === '⌫') {
            newAnswer = newAnswer.slice(0, -1);
       } else if (/^\d$/.test(key) && newAnswer.length < 2) {
           newAnswer += key;
       }
       setUserKeyboardInput(newAnswer);

       if (newAnswer.length > 0 && newAnswer === exerciseData?.answer) {
            if (newAnswer === exerciseData.answer) {
                processCorrectAnswer();
            } else {
                processIncorrectAnswer();
            }
  ...
  useEffect(() => {
    const saveScoreAndFetchHistory = async () => {
      if (isFinished && student && !hasBeenSaved && !isTableauMode) {
        setHasBeenSaved(true); // Prevent re-saving
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-2xl">
      {exerciseData.imageOptions?.map((option, index) => (
        <Button
          ...
          disabled={!!feedback}
        >
          <img src={option.src} alt={option.alt} className="max-h-24 object-contain" />
        </Button>
      ))}
    </div>
  );
        ...
        <CardHeader>
          <CardTitle className={cn(
           ...
           {exerciseData.syllable && (
                <CardDescription className="text-center font-bold text-5xl text-primary font-mono tracking-widest pt-2">
                    {exerciseData.syllable}
                </CardDescription>
            )}
        </CardHeader>
