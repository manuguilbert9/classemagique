...
// This interface is simplified to avoid circular dependencies.
// The main Question interface will hold all possible properties.
...
export async function generateCalendarQuestions(level: SkillLevel, count: number): Promise<Question[]> {
    ...
    while (questions.length < count) {
        ...
        if (!questionSet.has(newQ.question)) {
            ...
            questions.push(newQ as Question);
        }
    }
