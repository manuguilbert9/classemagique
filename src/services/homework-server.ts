...
import type { Score, ScoreDetail } from './scores';
...
export interface HomeworkResult extends Omit<Score, 'id' | 'skill' | 'createdAt'> {
    id?: string;
    userId: string;
    date: string; // Date of the homework assignment (e.g., "2024-09-16")
    skillSlug: string;
    score: number;
    /** Étapes, corrections et justifications des exercices progressifs. */
    details?: ScoreDetail[];
...
export async function saveHomework(dateId: string, assignments: Record<string, Assignment>): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await setDoc(homeworkDocRef, { assignments }, { mergeFields: ['assignments'] });
    return { success: true };
  } catch (error) {
...
export async function getHomeworkForStudent(
  ...
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      resultat.push({ date: docSnap.id, assignment: resolveAssignment({assignments: data.assignments || {}, assignmentsByStudent: data.assignmentsByStudent}, studentId, groupId)! });
    });
